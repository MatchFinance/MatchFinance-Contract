...
contract ProtocolRewardsPool is Ownable {
    ...
    constructor(address _config, address _eslbr) {
    ...
    function getClaimAbleLBR(address user) public view returns (uint256 amount) {
        if (time2fullRedemption[user] > lastWithdrawTime[user]) {
            amount = block.timestamp > time2fullRedemption[user] 
                ? unstakeRatio[user] * (time2fullRedemption[user] - lastWithdrawTime[user])
                : unstakeRatio[user] * (block.timestamp - lastWithdrawTime[user]);
        }
    }
    ...
    function getReward() external updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            uint256 eUSDAmount = balance >= reward ? reward : balance;
            eUSD.transfer(msg.sender, eUSDAmount);
            if(reward > eUSDAmount) {
                ...
                emit ClaimReward(msg.sender, eUSDAmount, address(0), 0, block.timestamp);
            }
