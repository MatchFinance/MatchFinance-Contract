...
contract esLBRBoost is Ownable {
    ...
    IRewardPool public miningIncentives;
    IERC20 public esLBR;
    ...
    constructor(address _miningIncentives, address _eslbr) {
        ...
        miningIncentives = IRewardPool(_miningIncentives);
        esLBR = IERC20(_eslbr);
    }
    ...
    function setMiningIncentives(address _addr) external onlyOwner {
        miningIncentives = IRewardPool(_addr);
    }
    ...
    function setLockStatus(uint256 id, uint256 lbrAmount, bool useLBR) external {
        ...
        require(esLBR.balanceOf(msg.sender) >= userStatus.lockAmount + lbrAmount, "IB");
    ...
    function getAmountNeedLocked(address user) public view returns (uint256) {
        ...
        return stakedAmount * esLBR.totalSupply() / totalStaked;
    }
}