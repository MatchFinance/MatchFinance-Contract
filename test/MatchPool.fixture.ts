...
export async function deployMPFixture() {
  ...
  const esLBR = await deploy("esLBR", []);
  ...
  const stakePool = await deploy("StakePool", [lp.address, esLBR.address]);

  const mining = await deploy("MiningIncentive", [configurator.address, lpOracle.address, "0x0000000000000000000000000000000000000000", esLBR.address]);
  await mining.setEthlbrStakeInfo(stakePool.address);
  await mining.setPool(mintPool.address);
  await configurator.setMining(mining.address);

  const boost = await deploy("esLBRBoost", [mining.address, esLBR.address]);
  await mining.setBoost(boost.address);
  ...
  await matchPool.setLybraBoost(boost.address);
  await matchPool.addMintPool(mintPool.address);
  await matchPool.setMonitor(admin.address);

  const manager = await deployUpgradeable("RewardManager", [matchPool.address], true);
  ...
  return { matchPool, stakePool, mining, mintPool, stETH, manager, admin, bob, esLBR, boost };
}